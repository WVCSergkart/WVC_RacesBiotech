...
namespace WVC_XenotypesAndGenes
{
	...
	public class Dialog_GolemTypesForSummon : Window
	{
		...
		public PawnKindDef selectedXeno;

		// public PawnKindDef currentXeno;

		public float rightViewWidth;

		public List<PawnKindDef> allXenotypes = new();
		...
		public Dialog_GolemTypesForSummon(Gene_Golemlink thisGene)
		{
			// Init
			gene = thisGene;
			...
			{
				selectedGolems.AddRange(allXenotypes);
			}
			// Settings
			forcePause = true;
			closeOnAccept = false;
		...
		{
			int count = 0;
			foreach (PawnKindDef allXenotype in allXenotypes)
			{
		...
		public void DrawLeftRect(Rect rect, ref float curY)
		{
			...
			// Log.Error("0");
			Widgets.Label(rect3.x, ref curY, rect3.width, selectedXeno.race.description);
			curY += 10f;
			// Log.Error("1");
			Widgets.HyperlinkWithIcon(new Rect(rect3.x, curY, rect3.width, Text.LineHeight), new Dialog_InfoCard.Hyperlink(selectedXeno.race));
			curY += 10f;
			Rect button = new(rect3.x, rect3.yMax - 85f, rect3.width, 25f);
			// Log.Error("2");
			if (!selectedGolems.Contains(selectedXeno))
			{
				if (Widgets.ButtonText(button, "WVC_XaG_AddToList".Translate()))
				{
					selectedGolems.Add(selectedXeno);
				}
			}
			else if (selectedGolems.Contains(selectedXeno))
			{
				if (Widgets.ButtonText(button, "WVC_XaG_RemoveFromList".Translate()))
				{
					selectedGolems.Remove(selectedXeno);
				}
			}
			// Log.Error("3");
		...
		public Color GetBoxColor(PawnKindDef mode)
		{
			Color result = TexUI.AvailResearchColor;
			if (selectedGolems.Contains(mode))
			{
		...
		public Color GetBoxOutlineColor(PawnKindDef mode)
		{
		...
		// public Color GetTextColor(PawnKindDef mode)
		// {
		...
		public void DrawStage(Rect rect, PawnKindDef stage, float count)
		{
			...
			Widgets.DefIcon(rect3.ContractedBy(4f), stage);
